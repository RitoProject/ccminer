/**
 * X16Rv2 algorithm (X16 with Randomized chain order)
 *
 * tpruvot 2018 - GPL code
 * penfold 2019 - add tiger192 before keccak, luffa and sha512
 */
...
// X16Rv2 CPU Hash (Validation)
extern "C" void x16rv2_hash(void *output, const void *input)
{
...
extern "C" int scanhash_x16rv2(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	switch (algo80) {
		...
		case JH:
			...
			break;
		case KECCAK:
            tiger192_setBlock_80(endiandata);
			break;
		case SKEIN:
...
// cleanup
extern "C" void free_x16rv2(int thr_id)
{
	if (!init[thr_id])
		return;
